...
namespace RopeToolkit
{
    ...
    public class Rope : MonoBehaviour
    {
        ...
        protected static void FillMeshFrames(ref NativeArray<float3> positions, ref NativeArray<float3> bitangents, Matrix4x4[] meshFrames, float spacing, bool isLoop, float rotationOffset)
        {
            var scale = Vector3.one * 0.5f * spacing;
        ...
        protected void SubmitToRenderer()
        {
            ...
            else
            {
                ...
                if (interpolation == RopeInterpolation.None)
                {
                    FillMeshFrames(ref positions, ref bitangents, customMeshFrames, _measurements.particleSpacing, isLoop, customMeshRotation);
                }
                else
                {
                    FillMeshFrames(ref interpolatedPositions, ref bitangents, customMeshFrames, _measurements.particleSpacing, isLoop, customMeshRotation);
                }
