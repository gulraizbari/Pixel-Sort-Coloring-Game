...
public class LevelData : ScriptableObject
{
    ...
    public void ChangeAllBrickSprites()
    {
        foreach (var stack in allStacks)
        {
            ...
            {
                foreach (var brick in data.brickData)
                {
    ...
    public void ReplaceColorInData(BrickColor newColor, BrickColor colorToReplace)
    {
        foreach (var stack in allStacks)
        {
            foreach (var data in stack.stackData)
            {
                var getBrick = data.brickData.Find(col => col.myBrick.brickColor == colorToReplace);
    ...
    public static Brick LoadBrickByBrickType(BrickColor Type)
    {
        var enemies = Resources.LoadAll<Brick>("Bricks");
...
public class StackData
{
    ...
    [BoxGroup("Stack")] public List<SubStack> brickData;
}
...
public class SubStack
{
    ...
    [PreviewField] public Sprite brickImage;
    private void UpdateBrickImage()
    {
        if (myBrick != null)
        {
            brickImage = myBrick.brickSprite;
        }
        else
        {
            brickImage = null;
